...
def test_gradcheck_dct(type, norm, size):
    ...
    assert torch.allclose(
        ...
        torch.as_tensor(scipy_dct(dat.numpy(), norm=norm, type=type)),
    )
...
def test_gradcheck_idct(type, norm, size):
    ...
    assert torch.allclose(
        ...
        torch.as_tensor(scipy_idct(dat.numpy(), norm=norm, type=type)),
    )
...
def test_gradcheck_dst(type, norm, size):
    ...
    assert torch.allclose(
        ...
        torch.as_tensor(scipy_dst(dat.numpy(), norm=norm, type=type)),
    )
...
def test_gradcheck_idst(type, norm, size):
    ...
    assert torch.allclose(
        ...
        torch.as_tensor(scipy_idst(dat.numpy(), norm=norm, type=type)),
    )
